...
class HandGestureProcessor {
    ...
    typealias PointsPair2 = (thumbTip: CGPoint, indexTip: CGPoint)
    ...
    private (set) var lastProcessedPointsPair2 = PointsPair2(.zero, .zero)
    
    init(pinchMaxDistance: CGFloat = 18, evidenceCounterStateTrigger: Int = 3) {
    ...
    func processPointsPair2(_ pointsPair2: PointsPair2) {
        lastProcessedPointsPair2 = pointsPair2
        let distance = pointsPair2.indexTip.distance(from: pointsPair2.thumbTip)
        if distance < pinchMaxDistance {
            // Keep accumulating evidence for pinch state.
            pinchEvidenceCounter += 1
            apartEvidenceCounter = 0
